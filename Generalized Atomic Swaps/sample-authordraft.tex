...
%% article.
\begin{abstract}
    Atomic swap is a protocol between two distrusted parties trying to swap two assets. The protocol ensures that if 1. If both the parties are to the protocol then the assets are swapped 2. If any or both the parties deviate from the protocol, then none of the parties end up being worse 3. No party can leave the other party in a deadlock, locking their asset. Moreover no party has any incentive to deviate from the protocol. 
    HTLC traditionally has been restricted to two parties, each exchanging their goods. In this paper we exam,one a general form of HTLC contract. We still restrict ourself to two ledger, but in each ledger there is a group of parties with a set of assets, which they want to swap. We first show that the traditional HTLC breaks down in case of general transfer. We then provide a novel protocol for atomic swap in case of traditional swap between two parties. This protocol does not have the time locking. We then presents a modified HTLC protocol, based on this protocol that conducts swaps in general case, even when some of the parties are byzantine.
